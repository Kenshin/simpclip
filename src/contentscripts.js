...
const root        = 'simpclip',
      ...
      code_icon   = '<svg t="1531708730344" class="icon" style="" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="1959" xmlns:xlink="http://www.w3.org/1999/xlink" width="14" height="14"><defs><style type="text/css"></style></defs><path d="M640 128l-384 768 128 0 384-768-128 0zm-512 128l-128 256 128 256 128 0-128-256 128-256-128 0zm640 0l128 256-128 256 128 0 128-256-128-256-128 0z" p-id="1960" fill="#ffffff"></path></svg>';
...
$body.on( 'mouseenter', 'pre', preEventHandler );
...
function preEventHandler( event ) {
    ...
    if ( [ 'pre' ].includes( type )) {
        if ( $body.find( 'simpclip' ).length > 0 ) remove();
        selection = { text: event.target.outerText, html: event.target.outerHTML, node: event.target, parent: event.target.parentNode };
        create( event, selection.html, type );
    }
}
 ...
 * @param {string} include: text, ( a | img ), pre
 */
function create( event, m_word, type ) {
    ...
    $( root ).on( 'click', 'simpclip-a', event => {
        ...
        if ( href == 'copy' ) {
            ...
            const result = copy( selection.html );
            result && new Notify().Render( "已成功复制到剪切板。" );
